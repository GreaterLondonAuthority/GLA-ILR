...
public class ESFMonthlyRecordFile {
    ...
    public static ESFMonthlyRecordFile parse(String name) {
        ...
        if (name.matches("SILR_+\\d{4}_\\d{2}+_LLDDHealthProblem(.csv)")) {
            ...
            esfMonthlyRecordFile.valid = esfMonthlyRecordFile.valid
                ? esfMonthlyRecordFile.year >= 1900 && esfMonthlyRecordFile.year <= 3000 : esfMonthlyRecordFile.valid;
        } else if (name.matches("(.*) +\\d{4} \\d{2}(.csv)")) {
    ...
    public static void validateESFMonthlyRecordFile(String name, DataImportType importType) {
        if (!parse(name).isValid()) {
            throw new IllegalArgumentException("Invalid file format: '" + importType.format + "'");
        }
    }
